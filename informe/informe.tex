...
A continuación se observa alguno de los casos mencionados anteriormente, por ejemplo como aún para valores bajos de k, alguno de los bordes de color blanco comienzan a tener una forma cuadriculada en vez de su forma original y como este pixelado se va haciendo mas notable.  
 \begin{figure}[H]
\centering
\subfloat[Original]{
...
A partir de lo resultados obtenidos podemos deducir que el tiempo de computo para este método no depende del tamaño de la imagen con la que se este trabajando ya que a veces la recontruccion de una imagen cuyo tamaño original es menor tarda más que una de mayor dimension. Pero, si se observa que en general el tiempo aumenta para valores de $\alpha$ menor, lo que posiblemente se deba a que en estos casos se realiza una mayor variación en la forma de conseguir el valor de un pixel, y dichas fluctuaciones repercuten notablemente en el tiempo de cómputo. 
...
A partir de los resultados obtenidos al comparar distintos métodos, nos encontramos con el hecho de que, para las imágenes con las que trabajamos, el método bilineal fue el que obtuvo en la mayoría de los casos mayor PSNR, y como consecuencia un menor ECM y cuando no sucedió entonces el método de Splines se sobrepuso. 
Pese a que creíamos que el método mas eficaz iba a ser el de Spline, esto no ocurrió. 
Primero analicemos lo sucedido con el método de vecino mas cercano, en este caso se cumplió lo que intuiamos, que no iba a ser mejor que los otros dos métodos para una imagen. Esto se puede deber a que es el método mas simple, el mismo no tiene un análisis fuerte a la hora de asignar el valor de un pixel desconocido. Funciona de manera golosa, donde el valor de un pixel tan solo va a ser igual al valor del pixel mas cercano, establecida por una vecindad $v$ de 4 pixel. Lo cual evidencia un notable error, que es creer el conjunto de puntos que utiliza a $v$ solo va a tener el valor de alguno los 4 pixels que la componen; siendo que en realidad podrían tomar de 0 -255 valores. Por esto es que los métodos de Spline o bilineal van a ser mejor ya que buscan de alguna manera "promediar$"$ entre distintos valores. 
Continuando, ahora en lo que respecta al análisis entre el Método bilineal y el de spline fue cuando obtuvimos resultados contrarios a lo esperado. A la hora de analizar las posibles causas de este hecho un posible factor influyente es que el método de Spline, es el que mayor cálculos realiza para asignar el valor a cada pixel, dado que se trabaja con aritmética finita cada calculo esta susceptible a tener un determinado margen de error, y si además cada resultado obtenido se utiliza en otros cálculos entonces el error se propaga y aumenta.  
Por lo que, con respecta a los métodos la implementación de Spline posee esta gran desventaja. En algunos casos este error puede ser lo suficientemente despreciable. comparado con los resultados obtenidos con otro método que utiliza menos cálculos pero, cuyos valores se alejan mas del valor que debería tener cada pixel; por ejemplo entre Spline y vecino mas cercano.
Otro factor influyente es el tipo de imagen con la que se trabaje, la disposición de los colores en la misma también pueden influir notablemente. La idea básica de ambos métodos, se basan en que si se tienen dos puntos conocidos P_i y P_k y se quieren calcular los valores P_t con i < t < k el método bilineal trazara una recta entre P_i y P_k donde los valores de los P_t serán definidos por los puntos que se definan por la pendiente de la misma. Mientras que en spline los puntos serán definidos por un polinomio cubico el cual permite que los valores a tomar no estén definidos de manera tan estricta sino que considera un mayor distribución de valores a tomar.
En imágenes donde los colores tienen una distribución similar, los resultados obtenidos por el método de spline y bilineal no deberían variar en demasía pero, dado el problema aritmético que se presenta en Spline hace que interfiera de manera nos despreciable causando el error observable. En cambio, si en la imágenes se presentan facciones de color muy variado entonces, según esta idea el método de spline debería ser mejor y vimos que esto sucedió en la imagen $Lago$ donde para todos los k utilizados el método de Spline fue superior. El mayor problema pudo estar en los bordes entre las secciones del cielo, la tierra y el lago (siendo la segunda de una tonalidad totalmente opuesta a las restantes). En el método de bilineal si se quiere calcular el valor de un pixel muy cercano a los bordes se vera fuertemente influido por un valor muy bajo y otro muy alto, alterando el valor del pixel respecto al que debería tener originalmente. En cambio en spline este valor depende de mas puntos y como tiene una traza mas suave nos va a permitir tener una mejor aproximación. 
Las imagenes con las que trabajamos nos permitieron plantear estas teorías luego de observar los resultados obtenidos, una posible experimentación seria trabar con mas imágenes que presenten similares características a la de imagen $Lago$ y ver si esto sigue sucediendo.
