...
double mas_cercano(int k, int dato_c, int dato_f, int i, int j, vector<vector <double> > &expandida)
{
	int datoSig_c = dato_c + k +1;
	int datoSig_f = dato_f + k + 1;
	if(dato_f == i)
...
/////////////cargada em img_reducida////////////////
    ...
    switch(metodo){
    	...
    	case VECINO:
    	{
			...
			ofstream salida("salidaVecino.csv",std::ofstream::out);// genero este nuevo archivo de salda con el formato pero el vector tiene valores cero
		...
		case SPLINE:
    	{
		    ...
		    for(int i = 0; i < expandida.size(); i += k + 1)		//salto de a k + 1 filas para no tocar las que tienen todos -1
		    {
		    	...
		    	for(int j = 1; j < expandida[i].size(); j += k + 1, p++)	//salto de a k + 1 columnas para no tocar los pixeles originales, con el proximo for voy rellenando los pixeles nuevos entre cada par de pixeles originales
		    	{
		    		...
		    		for(int t = 0; t < k; t++)
		    		{
			    		...
			    		double interpolacion = a_j + b_j * (t + 1) + c_j * (t + 1) * (t + 1) + d_j * (t + 1) * (t + 1) * (t + 1);
		    ...
		    for(int j = 0; j < expandida[0].size(); j++)
		    {
		    	...
		    	for(int i = 1; i < expandida.size(); i += k + 1, p++)
		    	{
		    		...
		    		for(int t = 0; t < k; t++)
		    		{
			    		double interpolacion = a_i + b_i * (t + 1) + c_i * (t + 1) * (t + 1) + d_i * (t + 1) * (t + 1) * (t + 1);
		...
		case BILINEAL:
    	{
			...
			ofstream salida("salidaBilineal.csv",std::ofstream::out);// genero este nuevo archivo de salda con el formato pero el vector tiene valores cero
		...
		case BILINEALBIS:
		{
		    ...
		    ofstream salida("salidaBilinealBis.csv",std::ofstream::out);// genero este nuevo archivo de salda con el formato pero el vector tiene valores cero
