...
double calculo_bilineal(int k, int dato_c, int dato_f, int i, int j, vector<vector <double> > &expandida)
{
	...
	double res = (double)((i - dato_f)*f_r1)/(double)(k+1) + (double)((datoSig_f - i)*f_r2)/(double)(k+1); //los numeradores estan al reves pero es porque la i aumenta a medida
...
double calculo_bilineal_por_filas(int k, int dato_c, int i, int j, vector<vector <double> > &expandida)
{
	...
	double res = ((double)expandida[i][dato_c] + (((double)expandida[i][datoSig_c] - (double)expandida[i][dato_c])/(double)(k+1))*((double)j-(double)dato_c));

	if(res < 255) {return res;}
	else {return 255;} //no estoy seguro de esto
}

double calculo_bilineal_por_columnas(int k, int dato_f, int i, int j, vector<vector <double> > &expandida)
{
	...
	double res = ((double)expandida[dato_f][j] + (((double)expandida[datoSig_f][j] - (double)expandida[dato_f][j])/(double)(k+1))*((double)i-(double)dato_f));

	if(res < 255) {return res;}
	else {return 255;} //no estoy seguro de esto
}

double calculo_bilineal_por_filasExpandido(int k, int dato_c, int i, int j, int cPixAIgnorar, vector<vector <double> > &expandida)
{
	int limite_c = expandida[0].size() - 1;
	int datoSig_c;
	...
	double res = ((double)expandida[i][dato_c] + (((double)expandida[i][datoSig_c] - (double)expandida[i][dato_c])/((double)(datoSig_c)-(double)(dato_c)))*((double)j-(double)dato_c));

	if(res < 255) {return res;}
	else {return 255;} //no estoy seguro de esto
}

double calculo_bilineal_por_columnasExpandido(int k, int dato_f, int i, int j, int cPixAIgnorar, vector<vector <double> > &expandida)
{
	int limite_f = expandida.size() - 1;
	int datoSig_f;
	...
	double res = ((double)expandida[dato_f][j] + (((double)expandida[datoSig_f][j] - (double)expandida[dato_f][j])/((double)(datoSig_f)-(double)(dato_f)))*((double)i-(double)dato_f));

	if(res < 255) {return res;}
	else {return 255;} //no estoy seguro de esto
}

void originBilineal(vector<vector<double> > &expandida, int k)
{
...
void porDiagBilineal(vector<vector<double> > &expandida, int k){
...
void expandidoBilineal(vector<vector<double> > &expandida, int k)
{
