...
int mas_cercano(int k, int dato_c, int dato_f, int i, int j, vector<vector <double> > &expandida)
{
...
void origin(vector<vector<double> > &expandida, int k)
{
	int dato_f, dato_c;
...
void dinamico(vector<vector<double> > &expandida, int k)
{
    for(int i = 0; i <  expandida.size(); i++)
    {
    	for(int j = 0; j < expandida[i].size() ; j++)
...
void vecinoProm(vector<vector<double> > &expandida){
